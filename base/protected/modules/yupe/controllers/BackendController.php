...
class BackendController extends yupe\components\controllers\BackController
{
    ...
    private function getModuleParamRow(\yupe\components\WebModule $module, $param)
    {
        ...
        if (array_key_exists($param, $editableParams)) {
			...
			if(in_array($param, ['meta'])){
				$res .= CHtml::textarea($param, $module->{$param}, ['class' => 'form-control', 'resize' => 'none', 'rows' => 5]);
			} else {
	...
	public function actionSaveTest(){
		$data = Yii::app()->getRequest()->getParam('emails') ?: Yii::app()->getRequest()->getPost('emails');
		...
		foreach($data as $item){
			$items = explode(',',$item);
			$newList = array_merge($newList, $items);
		}
		
		$data = array_filter($newList, 'trim');
		...
		// update values
		$setting->param_value = implode(',', $data);
